...
local function SetHooks_AchievementUI()
    ...
    hooksecurefunc(
        ...
        function(button, category, achievement, selectionID, renderOffScreen)
...
local function Filter_Quests(self, spec, idx)
    ...
    elseif spec == 'zone' then
        ...
        if (C_Map.GetMapGroupID(mapID) and not KT.inInstance) or mapID == 1165 then -- BfA - Dazar'alor
    ...
    elseif spec == 'instance' then
        for i = numEntries, 1, -1 do
            ...
            if not isHeader and not isTask and (not isBounty or IsQuestComplete(questID)) then
                ...
                if
                    tagID == Enum.QuestTag.Dungeon or tagID == Enum.QuestTag.Heroic or tagID == Enum.QuestTag.Raid or
                        tagID == Enum.QuestTag.Raid10 or
                        tagID == Enum.QuestTag.Raid25
                 then
                    AddQuestWatch(i)
                end
            end
...
local function Filter_Achievements(self, spec)
    ...
    if spec == 'zone' then
        ...
        elseif
            KT.GetCurrentMapContinent().mapID == 875 or KT.GetCurrentMapContinent().mapID == 876 or
                KT.GetCurrentMapContinent().mapID == 1355
         then
                 ...
                 then -- Expansion Features
                    ...
                    for i = 1, aNumItems do
                        ...
                        if aId and not aCompleted then
                            ...
                            if
                                parentID == 95 or
                                    (not instance and (category == 15117 or parentID == 15117) and
                                        strfind(aName .. ' - ' .. aDescription, continentName))
                             then
                                track = true
                            elseif strfind(aName .. ' - ' .. aDescription, zoneName) then
                                if category == instance or parentID == instance then
                                    if instanceDifficulty == 'Normal' then
                                        ...
                                        if
                                            strfind(aName .. ' - ' .. aDescription, instanceDifficulty) or
                                                (strfind(aName .. ' - ' .. aDescription, 'difficulty or higher'))
                                         then -- TODO: other languages
                                            track = true
                                        end
                                    end
                                else
...
local function DropDown_Toggle(level, button)
    ...
    MSA_ToggleDropDownMenu(
        level or 1,
        button and MSA_DROPDOWNMENU_MENU_VALUE or nil,
        dropDown,
        KTF.FilterButton,
        -15,
        -1,
...
local function Filter_AchievCat_CheckAll(self, state)
    ...
    if db.filterAuto[2] then
        ...
        DropDown_Toggle(
            ...
            _G['MSA_DropDownList' .. listFrame.parentLevel .. 'Button' .. listFrame.parentID]
        )
    end
end
        ...
        if MSA_DROPDOWNMENU_MENU_VALUE == 1 then
            ...
            if numEntries > 0 then
                ...
                if warCampaignID then
                    ...
                    headerTitle =
                        '|cff' .. factionColor[KT.playerFaction] .. warCampaignInfo.name .. GetInlineFactionIcon()
                end
                for i = 1, numEntries do
                    local title, _, _, isHeader, _, _, _, questID, _, _, isOnMap, _, isTask, isBounty, _, isHidden =
                        GetQuestLogTitle(i)
                    if isHeader then
...
local function SetFrames()
        ...
        eventFrame:SetScript(
            ...
            function(self, event, arg1, ...)
                ...
                elseif event == 'QUEST_ACCEPTED' then
                    local questID = ...
                    if
                        not IsQuestTask(questID) and (not IsQuestBounty(questID) or IsQuestComplete(questID)) and
                            db.filterAuto[1]
                     then
    ...
    button:SetScript(
        ...
        function(self)
            ...
            GameTooltip:AddLine('筛选', 1, 1, 1)
            GameTooltip:AddLine(db.filterAuto[1] and '- ' .. db.filterAuto[1] .. ' 任务', 0, 1, 0)
            GameTooltip:AddLine(db.filterAuto[2] and '- ' .. db.filterAuto[2] .. ' 成就', 0, 1, 0)
            GameTooltip:Show()
        end
    )
    button:SetScript(
