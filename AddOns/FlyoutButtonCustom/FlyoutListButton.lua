...
function FlyoutListButton:GetTextureEquipmentset()
	if (self.id == nil) then
		...
		local _, texture = C_EquipmentSet.GetEquipmentSetInfo(self.id)
		--print('GetEquipmentSetInfo texture', texture)
		return texture
	end
end
...
function FlyoutListButton:Set(command, value, subValue, id, macroValues)
	...
	if command then
		...
		elseif (command == "equipmentset") then
			...
			for k, v in pairs(equipmentSetIDs) do -- k - index, v - ID
				--print('equipmentSetIDs', k, v)
				local equipsetname = C_EquipmentSet.GetEquipmentSetInfo(v)
				--print(v, 'equipsetname', equipsetname)
				if value == equipsetname then
					--print('found')
					self.id = v
					break
				end
...
function FlyoutListButton:PostClick(button)
	...
	or (button == "RightButton" and FbcRightClickSet) then
		...
		if self.command then
			...
			elseif actBtnName:match("^NeuronActionButton%d+$") then
				--FlyoutButton_SetCursor(self.command, self.value, self.subValue, self.id)
				--actnBtn.MACRO_OnReceiveDrag(actnBtn)
				--ClearCursor()
