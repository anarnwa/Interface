...
do
    ...
    function ProcessDetection(detectionData)
        ...
        if
            not Data.Scanner.NPCs[npcID] or (throttleTime and now < throttleTime + detection.intervalSeconds) or
                (not detection.whileOnTaxi and _G.UnitOnTaxi('player'))
         then
            return
        end
...
function NPCScan:UpdateScanList(_, mapID)
    ...
    if
        profile.blacklist.mapIDs[scannerData.mapID] or
            profile.detection.continentIDs[scannerData.continentID] == Enum.DetectionGroupStatus.Disabled
     then
...
do
    ...
    fadeAnimationGroup:SetScript(
        'OnLoop',
        function(self, loopState)
