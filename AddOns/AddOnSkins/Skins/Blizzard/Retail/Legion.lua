...
function AS:Blizzard_ArtifactUI(event, addon)
    ...
    ArtifactFrame.AppearancesTab:HookScript(
        'OnShow',
        function(self)
    ...
    OrderHallTalentFrame:HookScript(
        ...
        function(self)
            pcall(
                function()
                    if
                        self.StyleFrame and self.StyleFrame.Background and self.StyleFrame.Background.GetTexture and
                            self.StyleFrame.Background:GetTexture()
                     then
                    ...
                    for i = 1, self:GetNumRegions() do
                        ...
                        if region and region:IsObjectType('Texture') then
                            if
                                not ((region == self.Background) or
                                    (self.Currency and self.Currency.Icon and region == self.Currency.Icon))
                             then
                                region:SetTexture()
                            end
                        end
                    ...
                    for i = 1, self:GetNumChildren() do
                        ...
                        if child and child.Icon and child.DoneGlow and not child.Backdrop then
                            ...
                            child.TalentDoneAnim:HookScript(
                                'OnFinished',
                                function()
