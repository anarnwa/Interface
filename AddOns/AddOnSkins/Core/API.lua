...
function AS:CreateShadow(Frame, NoRegister, Inverted)
    if (not AS:CheckOption('Shadows')) or Frame.Shadow then
        return
    end
...
function AS:Desaturate(frame)
    ...
    if frame.IsObjectType and (frame:IsObjectType('Button') or frame:IsObjectType('CheckButton')) then
        local Normal, Pushed, Highlight =
            frame:GetNormalTexture(),
            frame:GetPushedTexture(),
            frame:GetHighlightTexture()
...
function AS:SetTemplate(Frame, Template, Texture)
    ...
    if AS:CheckOption('ElvUIStyle', 'ElvUI') then
        if
            (Template == 'MerathilisUI' and AS:CheckAddOn('ElvUI_MerathilisUI')) or
                (Template == 'KlixUI' and AS:CheckAddOn('ElvUI_KlixUI'))
         then
            Frame:Styling()
        end
    end
...
function AS:SkinArrowButton(object, Arrow)
    if Arrow then
        ...
        if Name then
            if (strfind(Name, 'left') or strfind(Name, 'prev') or strfind(Name, 'decrement') or strfind(Name, 'back')) then
                Arrow = 'left'
            elseif
                (strfind(Name, 'right') or strfind(Name, 'next') or strfind(Name, 'increment') or
                    strfind(Name, 'forward'))
             then
                Arrow = 'right'
            elseif
                (strfind(Name, 'upbutton') or strfind(Name, 'top') or strfind(Name, 'asc') or strfind(Name, 'home') or
                    strfind(Name, 'maximize'))
             then
                Arrow = 'up'
            end
        end
    ...
    if object:IsObjectType('Button') then
        ...
        if not Button.Mask then
            ...
            Mask:SetTexture(
                [[Interface\AddOns\AddOnSkins\Media\Textures\Arrow]],
...
function AS:SkinCloseButton(Button, Reposition)
    ...
    local Mask = Button:CreateMaskTexture()
    Mask:SetTexture(
        [[Interface\AddOns\AddOnSkins\Media\Textures\Close]],
        'CLAMPTOBLACKADDITIVE',
        'CLAMPTOBLACKADDITIVE'
    )
    Mask:SetSize(10, 10)
...
function AS:SkinIconButton(Button)
    ...
    local ButtonName = Button.GetName and Button:GetName()
    local Icon, Texture =
        Button.icon or Button.Icon or ButtonName and (_G[ButtonName .. 'Icon'] or _G[ButtonName .. 'IconTexture'])
...
function AS:SkinRadioButton(Button)
    ...
    local Check, Highlight, Normal, Disabled =
...
function AS:SkinScrollBar(Frame)
    ...
    local ScrollUpButton =
        GrabScrollBarElement(Frame, 'ScrollUpButton') or GrabScrollBarElement(Frame, 'UpButton') or
        GrabScrollBarElement(Frame, 'ScrollUp') or
        GrabScrollBarElement(Parent, 'scrollUp')
    local ScrollDownButton =
        GrabScrollBarElement(Frame, 'ScrollDownButton') or GrabScrollBarElement(Frame, 'DownButton') or
        GrabScrollBarElement(Frame, 'ScrollDown') or
        GrabScrollBarElement(Parent, 'scrollDown')
    local Thumb =
        GrabScrollBarElement(Frame, 'ThumbTexture') or GrabScrollBarElement(Frame, 'thumbTexture') or
    ...
    Frame.Backdrop:SetPoint(
        'BOTTOMRIGHT',
        ScrollDownButton or Frame,
        ScrollUpButton and 'TOPRIGHT' or 'BOTTOMRIGHT',
        0,
        1
    )
...
function AS:StyleButton(Button)
    ...
    local Cooldown =
        Button:GetName() and _G[Button:GetName() .. 'Cooldown'] or Button.Cooldown or Button.cooldown or nil
    ...
    for _, child in pairs(childs) do
        if not child:GetName() then
            if not objType or (objType and child:IsObjectType(objType)) then
                ...
                if
                    childPoint1 == point1 and childParent == relativeTo and (not point2 or (childPoint2 == point2)) and
                        x == childX and
                        y == childY
                 then
                    return child
...
function AS:SkinIconAndTextWidget(widgetFrame)
end

function AS:SkinCaptureBarWidget(widgetFrame)
end
...
function AS:SkinDoubleStatusBarWidget(widgetFrame)
end

function AS:SkinIconTextAndBackgroundWidget(widgetFrame)
end

function AS:SkinDoubleIconAndTextWidget(widgetFrame)
end

function AS:SkinStackedResourceTrackerWidget(widgetFrame)
end

function AS:SkinIconTextAndCurrenciesWidget(widgetFrame)
end
...
function AS:SkinHorizontalCurrenciesWidget(widgetFrame)
end

function AS:SkinBulletTextListWidget(widgetFrame)
end

function AS:SkinScenarioHeaderCurrenciesAndBackgroundWidget(widgetFrame)
end

function AS:SkinTextureAndTextWidget(widgetFrame)
end

function AS:SkinSpellDisplay(widgetFrame)
end

function AS:SkinDoubleStateIconRow(widgetFrame)
end

function AS:SkinTextureAndTextRowWidget(widgetFrame)
end

function AS:SkinZoneControl(widgetFrame)
end
